    ...
    #region Resources
        ...
        public override void Create()
        {
                ...
                if (needsToUpdate)
                {
                    ...
                    for (int i = 0; i < 64; i++)
                    {
                        ...
                        m_BlueNoiseTextures[i] = (Texture2D)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Textures/BlueNoise/" + fileName, typeof(Texture2D));
                    }
                }
                if (cameraMotionVectorShader == null)
                    cameraMotionVectorShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/CameraMotionVectors.shader", typeof(Shader));
                if (objectMotionVectorShader == null)
                    objectMotionVectorShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/ObjectMotionVectors.shader", typeof(Shader));
                if (screenSpaceCS == null)
                    screenSpaceCS = (ComputeShader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/ScreenSpaceFeatures.compute", typeof(ComputeShader));

                if (HiZDepthShader == null)
                    HiZDepthShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/HiZDepth.shadergraph", typeof(Shader));
                if (ResolveLastFrameShader == null)
                    ResolveLastFrameShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/ResolveLastFrame.shadergraph", typeof(Shader));
                if (TraceSSRShader == null)
                    TraceSSRShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/TraceSSR.shadergraph", typeof(Shader));
                if (SSR_ResolveShader == null)
                    SSR_ResolveShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/SSR_Resolve.shadergraph", typeof(Shader));
                if (SSR_TemporalBlendingShader == null)
                    SSR_TemporalBlendingShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/SSR_TemporalBlending.shadergraph", typeof(Shader));
                if (SSR_UnpackIntoAccumShader == null)
                    SSR_UnpackIntoAccumShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/UnpackIntoAccum.shadergraph", typeof(Shader));
                if (SSR_UnpackIntoBufferShader == null)
                    SSR_UnpackIntoBufferShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/UnpackIntoBuffer.shadergraph", typeof(Shader));
                if (SSR_BilateralBlurShader == null)
                    SSR_BilateralBlurShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/SSR_BilateralBlur.shadergraph", typeof(Shader));
                if (TemporalClippingShader == null)
                    TemporalClippingShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/TemporalClipping.shadergraph", typeof(Shader));
                if (BilateralUpsamplingShader == null)
                    BilateralUpsamplingShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/BilateralUpsampling.shadergraph", typeof(Shader));
                if (postProcessingShader == null)
                    postProcessingShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/PostProcessing.shadergraph", typeof(Shader));
                if (SSAOShader == null)
                    SSAOShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/SSAO.shadergraph", typeof(Shader));
                if (BilateralBlurShader == null)
                    BilateralBlurShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/BilateralBlur.shadergraph", typeof(Shader)); 
                if (BloomShader == null)
                    BloomShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/Bloom.shadergraph", typeof(Shader)); 
                if (LutPrepareShader == null)
                    LutPrepareShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/LutPrepare.shadergraph", typeof(Shader)); 
            #endif
                                ...
                                in RenderingData renderingData)
        {
            ...
            if (screenSpaceCS == null)
                screenSpaceCS = (ComputeShader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/Shaders/ScreenSpaceFeatures.compute", typeof(ComputeShader));
            if (postProcessingShader == null)
                postProcessingShader = (Shader)UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/ShaderGraph Sample Demo/Scripts/ShaderGraphs/PostProcessing.shadergraph", typeof(Shader));
        #endif
